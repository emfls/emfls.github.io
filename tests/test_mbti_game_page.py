...
class MbtiGamePageTest(unittest.TestCase):
    ...
    def test_search_contract(self):
        ...
        for phrase in ("Free", "16-Type Personality Quiz", "20 Questions", "personality quiz"):
            self.assertIn(phrase.lower(), combined.lower())
        for phrase in ("2026-09-20", "entertainment", "No email or sign-up", "four preference pairs"):
            self.assertIn(phrase, self.html)
        self.assertEqual([item.get("@type") for item in self.page.json_ld].count("WebApplication"), 1)
        self.assertEqual([item.get("@type") for item in self.page.json_ld].count("FAQPage"), 1)
    ...
    def test_game_and_measurement_contract(self):
        ...
        for phrase in ("const questions", "const resultCount", "function showResult", "questions.length"):
            self.assertIn(phrase, self.html)
        self.assertEqual(self.html.count('q: "'), 20)
        self.assertNotIn("\n                },\n                },\n                {\n                    q:", self.html)
