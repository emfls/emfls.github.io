...
def test_percentage_calculator_is_safe_family():
    r=eligibility(candidate('퍼센트계산기', content_types='calculator/tool'), set(), set(), 0, 1, CFG)
...
def test_generic_and_ymyl_candidates_are_not_safe():
    assert eligibility(candidate('영어추천', content_types='informational'),set(),set(),0,1,CFG)['status']=='PAGE_REVIEW_READY'
    assert eligibility(candidate('세금계산기', content_types='calculator/tool'),set(),set(),0,1,CFG)['status']=='BLOCKED'

def test_duplicate_hold_and_limit_block():
    assert eligibility(candidate('퍼센트계산기'), {'/kor/util/x/'}, set(),0,1,CFG)['status']=='BLOCKED'
    assert eligibility(candidate('퍼센트계산기'), set(), {'퍼센트계산기'},0,1,CFG)['status']=='BLOCKED'
    assert eligibility(candidate('퍼센트계산기'), set(), set(),1,1,CFG)['status']=='BLOCKED'

def test_generator_is_deterministic_and_handles_invalid_input():
    html=generate_page(candidate('퍼센트계산기'),CFG)
    assert html == generate_page(candidate('퍼센트계산기'),CFG)
