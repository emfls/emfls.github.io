...
매일 오전 9시(Asia/Seoul)에 실제 검색·수익 근거를 읽고 신규 수익 후보 10~20개를 조사한 뒤, 기존 19,000여 페이지와 검색 intent가 겹치지 않는 강한 후보만 기본 3개, 최대 5개까지 자동 발행한다. 목표는 페이지 수가 아니라 WINNER 수, 신규 페이지 승률과 28일 평균 수익을 높이는 것이다.
...
- 매일 09:00 KST에 현재 로컬 프로젝트 작업을 재개한다.
...
Codex heartbeat는 매일 09:00 Asia/Seoul에 실행한다. 동일 task에 연결해 전일 결과와 진행 기록을 이어간다. 예약 prompt에는 데이터 확인, 후보 조사, 안전 게이트, 테스트, 커밋·push와 실패 시 중단 조건을 모두 포함한다.
