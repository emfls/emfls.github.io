...
- 2026-08-31: title/H1 중복 진단 5개 여행 페이지를 복구했다. 각 파일에 섞인 다른 도시 문서와 대화 로그를 제거하고 원래 도시 본문, URL, canonical, 광고·분석 코드를 보존했다.
- 회귀 검사 `tests/test_naver_duplicate_travel_documents.py`를 추가해 문서당 doctype, html, title, H1과 종료 태그가 하나인지 확인한다.
- 다음 구현은 description 누락 계산기 6개이며, 이후에는 네이버 노출 또는 GA4/AdSense 성과가 있는 페이지를 우선한다.
