#!/usr/bin/env python3
"""Apply the approved second 100-page GA4 quality batch."""
...
from tests.second_hundred_ga4_manifest import PAGES
...
def trust_copy(relative: str, category: str) -> tuple[str, str, str]:
    if category == "travel" and ("iraq-baghdad" in relative or "palestine-nablus" in relative):
        return (
            "분쟁·출입 제한과 여행경보를 당일 확인하세요",
            "2026년 8월 11일에 재검토했습니다. 이 페이지는 방문 허가나 안전을 보장하지 않습니다. 무력 충돌, 국경·검문 통제, 항공편, 통신과 영사 지원이 급변할 수 있으므로 대한민국 외교부 등 관할 정부의 최신 여행금지·여행경보와 현지 출입 규정을 당일 확인하세요.",
            "다른 여행지 정보",
        )
