...
def prepare_queue(rows, existing_urls=None, published_keywords=None, daily_limit=1, selected_at=None):
    return select_launch_candidate(rows, existing_urls, published_keywords, daily_limit, selected_at)
...
def main():
    ...
    result=prepare_queue(rows,{r.get('url') for r in index},{r.get('keyword') for r in published},1,args.selected_at)
