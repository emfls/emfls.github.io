...
def validate_config(config):
    ...
    for spec in config['families'].values():
        if not isinstance(spec,dict) or not isinstance(spec.get('enabled'),bool) or not isinstance(spec.get('allowedPatterns'),list) or not spec['allowedPatterns'] or not isinstance(spec.get('blockedTerms'),list) or spec.get('generator') not in GENERATORS or spec.get('validationContract') not in VALIDATORS: return False
    return True
