...
def select_launch_candidate(rows, existing_urls=None, published_keywords=None, daily_limit=1, selected_at=None, max_age_days=30):
    ...
    excluded={"duplicate_url":0,"duplicate_keyword":0,"similar_intent":0,"invalid_score":0,"stale_winner":0,"ymyl":0,"ineligible":0}
    eligible=[]; seen=[]
    for row in rows:
        ...
        if any(SequenceMatcher(None,norm,n).ratio() >= .86 for _,n in seen): excluded["similar_intent"]+=1; continue
        if _ymyl(row): excluded["ymyl"]+=1; continue
        seen.append((keyword,norm)); eligible.append(row)
    eligible.sort(key=lambda r:(-(1 if r.get("status") in {"WINNER","CANDIDATE"} else 0), -(1 if _tool(r) else 0), -float(r.get("opportunity_score") or 0), normalize_keyword(r.get("keyword"))))
    queue=[]
